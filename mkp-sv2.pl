...
my $pdfapi = "18090";
eval `cat pdfapi.pl`;
print "http://202.171.45.196:$pdfapi/pdf/api2/$ac/$pd\n";
my $json = get("http://202.171.45.196:$pdfapi/pdf/api2/$ac/$pd");
...
for my $sub (@{$subs}) {
  ...
  for my $chg (@{$chgs}){
   ...
   if($yt > $btm){
    ...
    #details data--------------------
    my $uv0 = ""; my $dtx = ""; my $ochg = 0.00; my $ocnt = 0;
    for my $usg (@{$usgs}){
    ...
    if(($dtx !~ /SMS/)){
     if( ($dtx !~ /MMS/) && ($dtx !~ /DATA/)){
       rLine();
       prText(500, $yt, cm5($ochg), 'right');
       prText(40, $yt, 'Called number: '.$ocnt);
       rLine();}}
   }
}
